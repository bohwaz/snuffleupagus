...
static void dump_config(void) {
  ...
  add_assoc_long(&arr, SP_TOKEN_GLOBAL "." SP_TOKEN_LOG_MEDIA, SPCFG(log_media).type);
  if (SPCFG(log_media).type == SP_LOG_FILE) {
    add_assoc_string(&arr, SP_TOKEN_GLOBAL "." SP_TOKEN_LOG_MEDIA, SPCFG(log_media).path);
  }
